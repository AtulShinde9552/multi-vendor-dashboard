...
export const add_banner = createAsyncThunk(
    ...
    async (info, { fulfillWithValue, rejectWithValue }) => {
        try {
            const { data } = await axios.post(`${base_url}/api/banner/add`, info, { withCredentials: true })
...
export const update_banner = createAsyncThunk(
    ...
    async ({ bannerId, info }, { fulfillWithValue, rejectWithValue }) => {
        try {
            const { data } = await axios.put(`${base_url}/api/banner/update/${bannerId}`, info, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const get_banner = createAsyncThunk(
    ...
    async (productId, { fulfillWithValue, rejectWithValue }) => {
        try {
            const { data } = await axios.get(`${base_url}/api/banner/get/${productId}`, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
