...
// import { get_regionaladmin } from '../../store/Reducers/chatReducer'
// import { BsEmojiSmile } from 'react-icons/bs'
// import toast from 'react-hot-toast'
// import { send_message_regionaladmin_admin, messageClear, get_admin_message ,updateRegionaladmin} from '../../store/Reducers/chatReducer'
...
//     const { regionaladmins, activeregionaladmin, regionaladmin_admin_message, currentRegionaladmin, successMessage } = useSelector(state => state.chat)
...
//     useEffect(() => {
//         if (successMessage) {
//             socket.emit('send_message_admin_to_regionaladmin', regionaladmin_admin_message[regionaladmin_admin_message.length - 1])
...
//     useEffect(() => {
//         socket.on('receved_regionaladmin_message', msg => {
...
//                 dispatch(updateRegionaladmin(recevedMessage))
//             }else{
...
//                                 <h2>regionaladmins</h2>
//                                 <span onClick={() => setShow(!show)} className='block cursor-pointer md:hidden'><IoMdClose /></span>
//                             </div>
//                             {
//                                 regionaladmins.map((s, i) => <Link key={i} to={`/admin/dashboard/chat-sellers/${s._id}`} className={`h-[60px] flex justify-start gap-2 items-center text-white px-2 rounded-sm py-2 rounded-sm cursor-pointer ${sellerId === s._id ? 'bg-slate-700' : ''}`}>
//                                     <div className='relative'>
//                                         <img className='w-[38px] h-[38px] border-white border-2 max-w-[38px] p-[2px] rounded-full' src={s.image} alt="" />
//                                         {
//                                             activeregionaladmin.some(a => a.sellerId === s._id) && <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
//                                         }
//                                     </div>
...
//                                     </div>
//                                     <span className='text-white'>{currentRegionaladmin?.name}</span>
//                                 </div>
//                             }
...
//                                 {
//                                     sellerId ? regionaladmin_admin_message.map((m, i) => {
